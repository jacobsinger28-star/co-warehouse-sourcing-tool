// Properties module — the off-market + on-market sourcing universe: filter rail,
// map / table / brokers views, and the property / broker-listings / email-composer
// drawers. Extracted from App.jsx (which is now a thin shell). App owns the
// persistent, top-bar-shared state (dataset, filters, channel, score, view, q,
// sourcing) and passes it in; this module owns its own transient view state
// (open drawer, current selection, map basemap, broker sort, in-progress email).
import { useEffect, useMemo, useRef, useState } from 'react'
import { css } from '../css.js'
import Icon from '../Icon.jsx'
import { MARKETS, SOURCES } from '../data.js'
import { DEMO } from '../demo.js'
import { addToQueue, removeFromQueue, isQueued } from '../callQueue.js'
...
// OR the mobile card list — instead of both.
...
export default function Properties({
  dataset, setDataset,
  filters, setFilters, setF, toggleSig, toggleInArr,
  channel, setChannel, score, setScore, view, setView, q, setQ, theme,
}) {
  ...
  const [railOpen, setRailOpen] = useState(false)    // mobile filter-rail drawer

  // Pipedrive write integration: token-configured? + in-flight + a transient toast.
  // In demo mode this hits /api/demo/pipedrive/* (simulated) — never a real CRM write.
  const [pdOk, setPdOk] = useState(null)   // null=checking, true/false=configured
  ...
  // match rows we hold — so the # LIST badge, the sort, and the drawer ALL read from
  // this map. That keeps the count honest: it never shows a phantom number that opens
  // an empty drawer.
  ...
  // anyone we can actually call (a phone or cell on file) floats to the top.
  ...
  // ── derived: filtering, selection, chips ──────────────────────────────────
  const ql = q.trim().toLowerCase()
  // memoized so unrelated state churn (the 3s/60s live-status poller in the shell)
  // keeps the array identity stable — otherwise DealMap re-renders every tick and
  // Leaflet tears down / re-adds markers, flickering any open popup
          ...
          <div className="card-list" style={css('flex-direction:column;flex:1;overflow-y:auto;min-height:0;')}>
            {visibleProps.map((p) => (
              <div key={p.id} className="hov" tabIndex={0} role="button" onClick={() => setDrawerId(p.id)} style={css(cardStyle(p.cat))}>
                ...
                <div style={css('display:flex;gap:16px;font-size:12px;color:var(--text2);flex-wrap:wrap;')}><span>{p.mkt}</span><span style={css('font-family:var(--mono);')}>{fmtSF(p.sf)} SF</span><span>{cardSub(p)}</span>{p.firstSeen && <span style={css('color:var(--text3);')}>Added {fmtDate(p.firstSeen)}</span>}{p.updated && fmtDate(p.updated) !== fmtDate(p.firstSeen) && <span style={css('color:var(--text3);')}>Updated {fmtDate(p.updated)}</span>}</div>
          ...
          <>
            ...
            <div className="detail-drawer" onClick={(e) => e.stopPropagation()} style={css('position:absolute;top:0;right:0;bottom:0;width:430px;background:var(--surface);border-left:1px solid var(--border2);z-index:26;display:flex;flex-direction:column;animation:drawerin .18s ease;box-shadow:-14px 0 40px rgba(0,0,0,.35);')}>
              <div style={css('flex:0 0 auto;padding:16px 18px;border-bottom:1px solid var(--border);')}>
                ...
                <div style={css('color:var(--text2);font-size:12.5px;margin-top:2px;')}>{drawerProp.mkt}, {drawerProp.st}{drawerProp.apn ? ` · APN ${drawerProp.apn}` : ''}{drawerProp.firstSeen ? ` · Added ${fmtDate(drawerProp.firstSeen)}` : ''}{drawerProp.updated ? ` · Updated ${fmtDate(drawerProp.updated)}` : ''}</div>
